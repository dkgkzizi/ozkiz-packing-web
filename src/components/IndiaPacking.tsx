...
type PackingItem = {
  ...
  size: string;
  qty: number;
  pdfQty: number; // ???®ÏúÑ ?êÎ≥∏ ?òÎüâ Ï∂îÍ?
};
...
export default function IndiaPacking() {
  ...
  const generateAndDownload = async (items: PackingItem[], originalName: string) => {
    ...
    const worksheet = workbook.addWorksheet('?∏ÎèÑÎß§Ïπ≠Í≤∞Í≥º');
    ...
    items.forEach(item => worksheet.addRow({ ...item, memo: `${dateStr}_?∏ÎèÑ ?ÖÍ≥†` }));
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="mb-12">
        ...
        <p className="text-slate-400 font-bold max-w-2xl leading-relaxed text-sm">
           ?ÖÎ¨¥ ?†Î¢∞?ÑÎ? Í∑πÎ??îÌïò??<span className="text-rose-600 font-black">???®ÏúÑ ?ïÎ? ?òÎüâ ?ÄÏ°??úÏä§??/span>???ëÏû¨?àÏäµ?àÎã§. <br />
           PDF ?êÎ≥∏Í≥??ëÏ? Í≤∞Í≥ºÍ∞íÏù¥ ?§ÏãúÍ∞ÑÏúºÎ°??ÅÌò∏ Í≤ÄÏ¶ùÎê©?àÎã§.
        </p>
      </header>
