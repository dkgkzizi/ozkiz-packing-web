...
const COLORS = ['BLACK','IVORY','WHITE','RED','BLUE','PINK','BROWN','NAVY','GREEN','YELLOW','BEIGE','GRAY','GREY','ORANGE','YELLOW','GOLD','SILVER','PURPLE','KHAKI','MINT','MELANGE','CHARCOAL','WINE','COCOA','LAVENDER','CORAL','PEACH', 'OATMEAL', 'CREAM', 'CHOCO'];
            ...
            if (isDataRow && !containsMetaKeyword) {
              ...
              if (dataCand) {
                ...
                for (const s of splitters) {
                  if (r.includes(s)) {
                    let pts = r.split(s).map((p: string)=>p.trim());
                    if (COLORS.some(cl => pts[0].toUpperCase().includes(cl))) { curC = pts[0]; curN = pts.slice(1).join(s).replace(')', '').trim(); }
                    else { curN = pts[0]; curC = pts.slice(1).join(s).replace(')', '').trim(); }
                    foundSplit = true; break;
                  }
                }
                if (!foundSplit) {
                  if (COLORS.some(cl => r.toUpperCase().includes(cl))) curC = r;
                  else curN = r;
                }
              }
