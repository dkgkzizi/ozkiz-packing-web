...
export default function PackingListApp() {
  ...
  // 자동 모드: 상태 변경 감지 후 다음 단계 자동 실행 (검증 단계는 두 파일 모두 필요)
  React.useEffect(() => {
    if (processingMode === 'auto' && autoStartNext && !loading) {
      ...
      if (isVerifyReady || isOtherReady) {
        ...
        // 짧은 지연을 주어 UI/상태 업데이트가 완료된 후 실행
  ...
  const startConversion = async () => {
    ...
    try {
      ...
      // --- Pipeline Logic (Manual/Auto Common) ---
    ...
    <main className="min-h-screen bg-[#0F172A] text-slate-200 font-sans selection:bg-indigo-500/30">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-12 md:py-20">
        ...
        <header className="mb-12 text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-black tracking-tighter text-white mb-4 bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-slate-500 uppercase">
            INDIA PACKING <span className="text-indigo-400 font-black">CONVERTER</span>
          </h1>
          <p className="text-slate-400 text-base md:text-lg max-w-2xl mx-auto leading-relaxed">
            인도 현지 패킹리스트의 디지털 전환. <br className="hidden md:block" />
            AI 데이터 추출부터 구글 시트 마스터 매칭까지 원클릭으로 완료하세요.
          </p>
        </header>
                ...
                <div 
                  ...
                  onDrop={(e) => {
                    ...
                    if (e.dataTransfer.files[0]) {
                      ...
                      setFile(dropped);
                      if (activeTab === 'convert') {
                        console.log('Original PDF Logged (Drop):', dropped.name);
                        setOriginalPdfFile(dropped);
                      }
                    }
                  }}
        ...
        {/* Footer */}
        <footer className="mt-20 pt-10 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4 text-slate-500 text-[10px] font-bold tracking-widest uppercase">
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-cyan-500 animate-pulse"></span>
            <span>All Systems Operational</span>
          </div>
          <div className="flex items-center gap-4">
            <span>© 2026 ANTIGRAVITY LOGISTICS INFRA</span>
            <span className="px-2 py-0.5 rounded bg-white/5 border border-white/10">v1.1.0</span>
          </div>
        </footer>
      </div>
    </main>
